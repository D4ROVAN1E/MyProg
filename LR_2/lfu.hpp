...
struct CacheNode {
    ...
    //Рабочий конструктор
    CacheNode(T v, int f, uint32_t ts)
        : value(v), frequency(f), lastUsedTimestamp(ts) {
    }
};
...
private:
    ...
    void evict() {
        ...
        if (lfuKeyIndexInArray != -1) {
            ...
            MDEL_BY_IND(keyStore, lfuKeyIndexInArray); //Удаляем из массива по индексу
        }
    }
...
public:
    ...
    void set(int keyInt, T value) {
        ...
        if (node != nullptr) {
            ...
            MPUSH_BACK(keyStore, key); //Добавляем ключ в наш массив для отслеживания
        }
    }
};