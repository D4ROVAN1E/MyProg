import sys
import os
import random
import ctypes
from typing import List

# Константы и таблицы для AES

# Таблица подстановок S-box
s_box = [
     ]

# Инвертированная таблица подстановок InvS-box
inv_s_box = [
     ]

# Раундовые константы Rcon
rcon = [
    0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 ]

# Вспомогательные функции

# Функция для печати байтового массива в HEX формате
def print_hex(label: str, data: List[int]):
    print(f"{label}: ", end="")
    for byte in data:
        print(f"{byte:02x} ", end="")
    print()

# Функция для печати состояния (State) в виде матрицы 4x4
def print_state(label: str, state: List[int]):
    print(f"--- {label} ---")
    for r in range(4):
        for c in range(4):
            # State заполняется по столбцам
            print(f"{state[c * 4 + r]:02x} ", end="")
        print()
    print("--------------------")

# Основные операции AES

# SubBytes: Замена каждого байта в state, используя s_box
def sub_bytes(state: List[int]):
    for i in range(16):
        state[i] = s_box[state[i]]

# InvSubBytes: Замена каждого байта в state, используя inv_s_box
def inv_sub_bytes(state: List[int]):
    for i in range(16):
        state[i] = inv_s_box[state[i]]

# ShiftRows: Циклический сдвиг строк state
def shift_rows(state: List[int]):
    temp = list(state)
    state[1] = temp[5]
    state[5] = temp[9]
    state[9] = temp[13]
    state[13] = temp[1]
    
    state[2] = temp[10]
    state[10] = temp[2]
    state[6] = temp[14]
    state[14] = temp[6]
    
    state[3] = temp[15]
    state[15] = temp[11]
    state[11] = temp[7]
    state[7] = temp[3]


# InvShiftRows: Обратный циклический сдвиг строк state
def inv_shift_rows(state: List[int]):
    temp = list(state)
    state[5] = temp[1]
    state[9] = temp[5]
    state[13] = temp[9]
    state[1] = temp[13]
    
    state[10] = temp[2]
    state[2] = temp[10]
    state[14] = temp[6]
    state[6] = temp[14]
    
    state[15] = temp[3]
    state[11] = temp[15]
    state[7] = temp[11]
    state[3] = temp[7]

# Умножение в поле Галуа GF(2^8)
def gmul(a: int, b: int):
    p = 0
    a &= 0xFF  # Ограничиваем a 8 битами
    b &= 0xFF  # Ограничиваем b 8 битами
    for _ in range(8):
        if b & 1:
            p ^= a
        hi_bit_set = a & 0x80
        a <<= 1
        a &= 0xFF  # Keep it in 8 bits
        if hi_bit_set:
            a ^= 0x1b # x^8 + x^4 + x^3 + x + 1
        b >>= 1
    return p & 0xFF  # Возвращаем результат, ограниченный 8 битами

# MixColumns: Смешивание столбцов state
def mix_columns(state: List[int]):
    temp = list(state)
    for c in range(4):
        i = c * 4
        temp[i] = gmul(0x02, state[i]) ^ gmul(0x03, state[i + 1]) ^ state[i + 2] ^ state[i + 3]
        temp[i + 1] = state[i] ^ gmul(0x02, state[i + 1]) ^ gmul(0x03, state[i + 2]) ^ state[i + 3]
        temp[i + 2] = state[i] ^ state[i + 1] ^ gmul(0x02, state[i + 2]) ^ gmul(0x03, state[i + 3])
        temp[i + 3] = gmul(0x03, state[i]) ^ state[i + 1] ^ state[i + 2] ^ gmul(0x02, state[i + 3])
    
    for i in range(16):
        state[i] = temp[i] & 0xFF  # Добавляем ограничение по байту

# InvMixColumns: Обратное смешивание столбцов state
def inv_mix_columns(state: List[int]):
    temp = list(state)
    for c in range(4):
        i = c * 4
        temp[i] = gmul(0x0e, state[i]) ^ gmul(0x0b, state[i + 1]) ^ gmul(0x0d, state[i + 2]) ^ gmul(0x09, state[i + 3])
        temp[i + 1] = gmul(0x09, state[i]) ^ gmul(0x0e, state[i + 1]) ^ gmul(0x0b, state[i + 2]) ^ gmul(0x0d, state[i + 3])
        temp[i + 2] = gmul(0x0d, state[i]) ^ gmul(0x09, state[i + 1]) ^ gmul(0x0e, state[i + 2]) ^ gmul(0x0b, state[i + 3])
        temp[i + 3] = gmul(0x0b, state[i]) ^ gmul(0x0d, state[i + 1]) ^ gmul(0x09, state[i + 2]) ^ gmul(0x0e, state[i + 3])
    
    for i in range(16):
        state[i] = temp[i] & 0xFF  # Добавляем ограничение по байту

# AddRoundKey: XOR состояния с раундовым ключом
def add_round_key(state: List[int], round_key: List[int]):
    for i in range(16):
        state[i] ^= round_key[i]
        state[i] &= 0xFF # Добавляем ограничение по байту

# Расширение ключа

# Функция расширения ключа AES-128
def key_expansion(key: List[int]) -> List[int]:
    expanded_keys = [0] * 176
    
    # Копируем исходный ключ в начало expanded_keys
    expanded_keys[:16] = key[:16]

    bytes_generated = 16
    rcon_iteration = 1
    temp = [0] * 4

    while bytes_generated < 176:
        # Берем последние 4 байта из сгенерированных
        temp = expanded_keys[bytes_generated - 4 : bytes_generated]

        # Если это начало нового раундового ключа (каждые 16 байт)
        if bytes_generated % 16 == 0:
            # RotWord: циклический сдвиг байтов влево
            t = temp[0]
            temp[0] = temp[1]
            temp[1] = temp[2]
            temp[2] = temp[3]
            temp[3] = t

            # SubWord: применяем S-box к каждому байту
            for i in range(4):
                temp[i] = s_box[temp[i]]

            # XOR с Rcon
            temp[0] ^= rcon[rcon_iteration]
            rcon_iteration += 1

        # Генерируем следующие 4 байта ключа
        for i in range(4):
            expanded_keys[bytes_generated] = (expanded_keys[bytes_generated - 16] ^ temp[i]) & 0xFF # Добавляем ограничение по байту
            bytes_generated += 1
            
    return expanded_keys

# Шифрование и дешифрование одного блока

def encrypt_block(block: List[int], expanded_keys: List[int]):
    print_state("Исходное состояние блока", block)
    add_round_key(block, expanded_keys[0:16])

    for round_num in range(1, 10):
        print_state(f"Начало раунда {round_num}", block)
        sub_bytes(block)
        print_state("После SubBytes", block)
        shift_rows(block)
        print_state("После ShiftRows", block)
        mix_columns(block)
        print_state("После MixColumns", block)
        add_round_key(block, expanded_keys[round_num * 16 : round_num * 16 + 16])

    # Final round (без MixColumns)
    print_state("Начало финального раунда (10)", block)
    sub_bytes(block)
    print_state("После SubBytes", block)
    shift_rows(block)
    print_state("После ShiftRows", block)
    add_round_key(block, expanded_keys[10 * 16 : 10 * 16 + 16])
    print_state("Зашифрованный блок", block)

def decrypt_block(block: List[int], expanded_keys: List[int]):
    print_state("Исходный шифроблок", block)
    add_round_key(block, expanded_keys[10 * 16 : 10 * 16 + 16])

    for round_num in range(9, 0, -1):
        print_state(f"Начало раунда {round_num}", block)
        inv_shift_rows(block)
        print_state("После InvShiftRows", block)
        inv_sub_bytes(block)
        print_state("После InvSubBytes", block)
        add_round_key(block, expanded_keys[round_num * 16 : round_num * 16 + 16])
        print_state("После AddRoundKey", block)
        inv_mix_columns(block)
        print_state("После InvMixColumns", block)

    # Final round (без InvMixColumns)
    print_state("Начало финального раунда (0)", block)
    inv_shift_rows(block)
    print_state("После InvShiftRows", block)
    inv_sub_bytes(block)
    print_state("После InvSubBytes", block)
    add_round_key(block, expanded_keys[0:16])
    print_state("Расшифрованный блок", block)

# Режим CBC

# Дополнение PKCS#7
def pad_pkcs7(data: List[int]) -> List[int]:
    pad_len = 16 - (len(data) % 16)
    if pad_len == 0:
        pad_len = 16
    return data + [pad_len] * pad_len

# Удаление дополнения PKCS#7
def unpad_pkcs7(data: List[int]) -> List[int]:
    if not data:
        return []
    pad_len = data[-1]
    if pad_len == 0 or pad_len > 16 or pad_len > len(data):
        return [] # Возвращаем пустой список, чтобы имитировать ошибку
    
    for i in range(pad_len):
        if data[len(data) - 1 - i] != pad_len:
            return []
            
    return data[:len(data) - pad_len]

def aes_cbc_encrypt(plaintext: List[int], key: List[int], iv: List[int]) -> List[int]:
    expanded_keys = key_expansion(key)

    print("\n>>> ГЕНЕРАЦИЯ РАУНДОВЫХ КЛЮЧЕЙ (для шифрования) <<<")
    for i in range(11):
        print_hex(f"Ключ раунда {i}", expanded_keys[i * 16 : i * 16 + 16])

    padded_plaintext = pad_pkcs7(plaintext)

    ciphertext = []
    prev_block = list(iv)

    print("\n>>> ПРОЦЕСС ШИФРОВАНИЯ CBC <<<")
    for i in range(0, len(padded_plaintext), 16):
        print(f"\n--- Шифрование блока {(i // 16) + 1} ---")
        current_block = padded_plaintext[i : i + 16]

        # XOR с предыдущим зашифрованным блоком (или IV)
        for j in range(16):
            current_block[j] ^= prev_block[j]
            current_block[j] &= 0xFF  # Ограничиваем результат байтом

        encrypt_block(current_block, expanded_keys)
        
        ciphertext.extend(current_block)
        prev_block = list(current_block) # Создаем копию

    return ciphertext

def aes_cbc_decrypt(ciphertext: List[int], key: List[int], iv: List[int]) -> List[int]:
    expanded_keys = key_expansion(key)

    print("\n>>> ГЕНЕРАЦИЯ РАУНДОВЫХ КЛЮЧЕЙ (для дешифрования) <<<")
    for i in range(11):
        print_hex(f"Ключ раунда {i}", expanded_keys[i * 16 : i * 16 + 16])

    plaintext = []
    prev_block = list(iv)

    print("\n>>> ПРОЦЕСС ДЕШИФРОВАНИЯ CBC <<<")
    for i in range(0, len(ciphertext), 16):
        print(f"\n--- Дешифрование блока {(i // 16) + 1} ---")
        current_block = ciphertext[i : i + 16]
        
        # Сохраняем копию шифроблока для следующего раунда
        encrypted_block_copy = list(current_block)

        decrypt_block(current_block, expanded_keys)

        # XOR с предыдущим зашифрованным блоком (или IV)
        for j in range(16):
            current_block[j] ^= prev_block[j]
            current_block[j] &= 0xFF # Ограничиваем результат байтом

        plaintext.extend(current_block)
        prev_block = encrypted_block_copy

    decrypted_text = unpad_pkcs7(plaintext)
    if not decrypted_text:
        print("Ошибка: неверное дополнение (padding)!", file=sys.stderr)
    
    return decrypted_text


# --- Главная функция ---

def main():
    plain_text_str = "Тестовое сообщение для AES CBC. Test message for AES CBC!"
    
    plaintext = list(plain_text_str.encode('utf-8'))

    # Генерация случайного ключа и вектора инициализации (IV)
    key = [random.randint(0, 255) for _ in range(16)]
    iv = [random.randint(0, 255) for _ in range(16)]

    #Запись ключа в файл
    try:
        with open("key.txt", "w") as key_file:
            for byte in key:
                key_file.write(f"{byte:02x}")
        print("Случайный ключ был сгенерирован и записан в key.txt")
    except IOError:
        print("Не удалось открыть файл key.txt для записи.", file=sys.stderr)
        
    print("\t\tAES-128 CBC ДЕМОНСТРАЦИЯ\n\n\n")

    print(f"Исходный текст: {plain_text_str}")
    print_hex("Исходный ключ (128 бит)", key)
    print_hex("Вектор инициализации (IV)", iv)
    print("\n--------------------------------------------------------------------")

    #Шифрование
    ciphertext = aes_cbc_encrypt(plaintext, key, iv)
    print("\n\t\tРЕЗУЛЬТАТ ШИФРОВАНИЯ")
    print("Зашифрованный текст (HEX):")
    for i, byte in enumerate(ciphertext):
        print(f"{byte:02x} ", end="")
        if (i + 1) % 16 == 0:
            print()
    print("\n--------------------------------------------------------------------")
    
    #Дешифрование
    decrypted_bytes = aes_cbc_decrypt(ciphertext, key, iv)
    if decrypted_bytes:
        # Список чисел сначала преобразуется в объект bytes, а затем декодируется
        decrypted_str = bytes(decrypted_bytes).decode('utf-8')
        print("\n\t\tРЕЗУЛЬТАТ ДЕШИФРОВАНИЯ")
        print(f"Расшифрованный текст: {decrypted_str}")

    print("\n--------------------------------------------------------------------")

    #Проверка
    # Сравниваем исходную строку с расшифрованной
    if 'decrypted_str' in locals() and plain_text_str == decrypted_str:
        print("ПРОВЕРКА ПРОЙДЕНА: Исходный и расшифрованный тексты совпадают.")
    else:
        print("ПРОВЕРКА НЕ ПРОЙДЕНА: Тексты не совпадают!")


if __name__ == "__main__":
    main()